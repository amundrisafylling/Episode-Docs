...
![Upload File Flow](../../../../images/flow/upload-file-flow.png)
...
| Target Directory | No | Directory to store the file in. |
| File name        | Yes | The name file to upload. |
| File Data        | Yes | The source for the file content. It must be a byte array or [Stream](https://learn.microsoft.com/en-us/dotnet/api/system.io.stream). |
| Overwrite        |           | If not checked, the action will fail if a file with the same name already exists. |
| Result variable name | No | Add a name if You want to get an result object with information on e.g. the new file id for the uploaded file. | 