...
| Work Process Version | Required | The ID of the Work Process Version to close. You can select from a list or enter a dynamic expression. |
...
| Result variable name | Optional | Name of a Flow variable that will receive `true` if the version was closed successfully, or `false` if the operation failed. |
