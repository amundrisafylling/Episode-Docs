...
> If the tabular data is present on a nested field instead of the root level, you must specify the [Root path](#root-path) (see properties below).
...
This Flow retrieves employees from the PowerOfficeGo REST API (in a JSON format), transforms it to a tabular format (row and columns), and inserts it into a database.
...
| DataReader variable name | Optional | The name of the variable that contains the output from this action. | 
